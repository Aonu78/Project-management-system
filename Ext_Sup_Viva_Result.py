...
def data_collector():
    ...
    cmd = conn.cursor()
    cmd.execute('''SELECT * from External_Supervisor_Viva_Result_Table''');
    data_list = []
    data_free = []
    lok = 65
    i = 1
...
def STUDENT_FORM_SUBMIT():
    ...
    cur = conn.cursor()
    cur.execute('insert into External_Supervisor_Viva_Result_Table (Department_Name,Program,'
                'Evaluation_Year,Session,Timing,Registration_No,External_Name,Phase,Student_Name,'
                'Enrollment,Internal_Name,Project_Name,Marks,GPA,Grade) values (?,?,?,?,?,?,?,?,?,'
                '?,?,?,?,?,?)', (Department_Entry.get(), Program_entry.get(), Evaluation_Year_entry.get(),
                               Session_entry.get(), Timing_entry.get(), Reistration_entry.get(),
                               External_Name_entry.get(), Phase_entry.get(),
...
class Viva_Result(Frame):
    def __init__(self, window):
        ...
        Data_frame.grid(row=1, column=0, columnspan=3,rowspan=2,ipady=10)
        ...
        Department_Entry.grid(row=0, column=1, pady=10)
        ...
        global Reistration_entry
        Reistration_Label = Label(Data_frame, text="Reistration No", font=('Helvetica', 12, 'bold'))
        Reistration_Label.grid(row=5, column=0)
        Reistration_entry = Entry(Data_frame, borderwidth=3, width=20, font=('Helvetica', 12, 'bold'))
        Reistration_entry.grid(row=5, column=1, pady=10)
        global External_Name_entry
        External_Name_Label = Label(Data_frame, text="External Name", font=('Helvetica', 12, 'bold'))
        External_Name_Label.grid(row=6, column=0)
        External_Name_entry = Entry(Data_frame, borderwidth=3, width=20, font=('Helvetica', 12, 'bold'))
        External_Name_entry.grid(row=6, column=1, pady=10)
        global Phase_entry
        Phase_Label = Label(Data_frame, text="Phase", font=('Helvetica', 12, 'bold'))
        Phase_Label.grid(row=7, column=0)
        Phase_entry = Entry(Data_frame, borderwidth=3, width=20, font=('Helvetica', 12, 'bold'))
        Phase_entry.grid(row=7, column=1, pady=10)

        # frame 2

        Data_frame1 = LabelFrame(self.Student_Zone_Label, text="External Supervisor Result",
                                font=('Helvetica', 16, 'bold'))
        ...
        Student_Name_Entry.grid(row=0, column=1, pady=12)
        ...
        Enrollment_entry.grid(row=1, column=1, pady=12)
        ...
        Internal_Name_entry.grid(row=2, column=1, pady=12)
        ...
        Project_Name_entry.grid(row=3, column=1, pady=12)
        global Marks_entry
        ...
        Marks_entry.grid(row=4, column=1, pady=12)
        global GPA_entry
        ...
        GPA_entry.grid(row=5, column=1, pady=12)
        global Grade_entry
        ...
        Grade_entry.grid(row=6, column=1, pady=12)
        # end
